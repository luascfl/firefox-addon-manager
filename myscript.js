...
const selfExtensionId = extensionApi.runtime.id;
let firefoxToggleWarningShown = false;
const addonBaseUrl = extensionApi.runtime.getURL('');
/****** END DECLARING VARIABLES ******/

function callSetEnabled(id, enabled) {
...
function showFirefoxToggleWarning() {
	if (firefoxToggleWarningShown) {
		return;
	}

	firefoxToggleWarningShown = true;
	Materialize.toast(
		'Firefox only lets enterprise-installed add-ons toggle others. Actions might not apply.',
		4000,
		'alert'
	);
}
...
function sanitizeId(id) {
	return `ext-${id.replace(/[^a-zA-Z0-9_-]/g, '-')}`;
}

function findSwitchByExtensionId(id) {
	return $('.switch.ext-switch').filter(function () {
		return $(this).data('extid') === id;
	});
}
...
function toggleExtensions(ids, enabled) {
	...
	return Promise.allSettled(ids.map(function (id) {
		return callSetEnabled(id, enabled);
	}));
}

function warnIfRejected(results) {
	if (results.some(function (result) { return result.status === 'rejected'; })) {
		showFirefoxToggleWarning();
	}
}


const policyCallout = document.getElementById('policy-callout');

if (policyCallout) {
	extensionApi.management.getSelf(function (selfInfo) {
		const err = extensionApi.runtime.lastError;
		if (err) {
			console.warn('Unable to verify install type', err);
			policyCallout.style.display = 'block';
			return;
		}
		if (selfInfo && selfInfo.installType === 'admin') {
			policyCallout.style.display = 'none';
		} else {
			policyCallout.style.display = 'block';
		}
	});
...
chrome.management.getAll(function(info) {
	...
	for (let i = 0; i < extArray.length; i++) {
		...
		entry.domId = sanitizeId(entry.id);

		if (entry.icons && entry.icons.length) {
			let chosenIcon = '';
			for (let x = entry.icons.length - 1; x >= 0; x--) {
				let iconUrl = entry.icons[x].url;
				if (!iconUrl) {
					continue;
				}

				const isCrossExtensionMozUrl =
					iconUrl.startsWith('moz-extension://') && !iconUrl.startsWith(addonBaseUrl);

				if (!isCrossExtensionMozUrl) {
					chosenIcon = iconUrl;
					break;
				}
			}
			entry.pic = chosenIcon || 'images/icon-128.png';
		} else {
			entry.pic = 'images/icon-128.png';  // fallback icon
		}
	...
	// hide on/off switch for this extension itself
	findSwitchByExtensionId(selfExtensionId).find('label').hide();

	// turn on/off extensions when toggle is switched
	$('.state-switch').change(function () {
		$('#refresh-icon').show();
		const switchContainer = $(this).parents('.switch');
		// get the app id
		let id = switchContainer.data('extid'),
		// get the app name
		name = switchContainer.attr('name');
		let togglePromise = Promise.resolve();
		const checkbox = this;
		for (let i = 0; i < extArray.length; i++) {
			if (extArray[i].id === id && !extArray[i].enabled) {
				extArray[i].enabled = true;
				togglePromise = callSetEnabled(id, true).then(function () {
					Materialize.toast(`${name} is now on`, 2000, 'ccToastOn');
				}).catch(function (error) {
					extArray[i].enabled = false;
					checkbox.checked = false;
					console.warn('Failed to enable', name, error);
					showFirefoxToggleWarning();
					Materialize.toast(`Unable to enable ${name}: ${error.message}`, 4000, 'alert');
				});
				break;
			}
			else if (extArray[i].id === id && extArray[i].enabled) {
				extArray[i].enabled = false;
				togglePromise = callSetEnabled(id, false).then(function () {
					Materialize.toast(`${name} is now off`, 2000, 'ccToastOff');
				}).catch(function (error) {
					extArray[i].enabled = true;
					checkbox.checked = true;
					console.warn('Failed to disable', name, error);
					showFirefoxToggleWarning();
					Materialize.toast(`Unable to disable ${name}: ${error.message}`, 4000, 'alert');
				});
				break;
			}
		}
...
function addExtensions(name) {
	...
	for (let i = 0; i < extArray.length; i++) {
		...
		if (extArray[i].id === `${selfExtensionId}`) {
			continue;
		}
		let ext = extArray[i];
...
function submitThatShit() {
	...
	let togglePromises = Promise.resolve();
	if (currentJob === 'creating' && idList.length) {
		togglePromises = Promise.all(idList.map(function(extensionId) {
			return callSetEnabled(extensionId, true).catch(function (error) {
				console.warn('Failed to enable extension', extensionId, error);
				showFirefoxToggleWarning();
			});
		}));
	}
	...
	togglePromises.finally(function () {
		setTimeout(function () {
...
function addGroupLabels(groups){
	...
	$('.groupLabel-on, .groupLabel-off').remove();
	...
	for (let group in groups) {
		...
		groups[group].forEach(function (id) {
			let switchEl = findSwitchByExtensionId(id);
			if (!switchEl.length) {
				return;
			}
			let target = switchEl.closest('.extBlock').find('.extName');
...
function template(entry) {
	...
	<div class="righty">
    <div class="switch ext-switch" id="${entry.domId}" data-extid="${entry.id}" name="${entry.name}">
	    <label>
...
function extListTemplate(ext) {
	let toggleId = sanitizeId(`${ext.id}-toggle`);
	return `
	<div class='extList-holder'>
		<div class='extList-toggle'>
			<input type="checkbox" id="${toggleId}" data-appid="${ext.id}">
...
$("#searchbox").keyup(function () {
	...
	setTimeout(function () {
		if ($(".extName:visible").length === 0) {
			...
			// update url
			$('#noResults a.filterLink').attr("href", `https://addons.mozilla.org/firefox/search/?q=${encodeURIComponent(searchTerm)}`);
		} else {
			$("#noResults").hide();
...
$("body").on("click", ".group-btn", function () {
	...
	if (btn.hasClass("on")) {
		...
		if (extsInThisGroup.length === inOthers.length) {
			...
			let idsToDisable = user.groups[groupClicked].filter(function (extensionId) {
				return $.inArray(extensionId, inOthers) === -1;
			});
			toggleExtensions(idsToDisable, false).then(function (results) {
				warnIfRejected(results);
				if (results.every(function (result) { return result.status === 'fulfilled'; })) {
					Materialize.toast(`${groupClicked} updates applied`, 2000, 'ccToastOff');
				} else {
					Materialize.toast(`Some extensions in ${groupClicked} could not be disabled.`, 4000, 'alert');
				}
			}).finally(function () {
				addGroupLabels(user.groups);
				setTimeout(function () {
			...
			toggleExtensions(user.groups[groupClicked], false).then(function (results) {
				warnIfRejected(results);
				if (results.every(function (result) { return result.status === 'fulfilled'; })) {
		...
		toggleExtensions(user.groups[groupClicked], true).then(function (results) {
			warnIfRejected(results);
			if (results.every(function (result) { return result.status === 'fulfilled'; })) {
				Materialize.toast(`${groupClicked} is now on`, 2000, 'ccToastOn');
				btn.removeClass("off").addClass("on");
			} else {
				Materialize.toast(`Some extensions in ${groupClicked} could not be enabled.`, 4000, 'alert');
			}
		}).finally(function () {
			setTimeout(function () {
				location.reload(false);
...
$("body").on("click", ".edit", function () {
	...
	for (let i = user.groups[groupName].length-1; i >= 0; i--) {
		...
		$(`#editExtList input[data-appid="${id}"]`).prop('checked', true);
		$("#editExtList").prepend($(`[data-appid="${id}"]`)[0].parentElement.parentElement);
	}
...
function removeSelfExtension() {
	Object.keys(user.groups).forEach(function (key) {
		user.groups[key] = arrayRemove(user.groups[key], `${selfExtensionId}`);
...
// remove an item from an array (used for removing this extension from users' groups)
